...
{
    public class InsertCountryRequestValidator : AbstractValidator<InsertCountryRequest>
    {
        private readonly KalaDbContext _context;
        public InsertCountryRequestValidator(KalaDbContext context)
        {
            _context = context;
        ...
        private async Task<bool> BeUniqueCountryName(string countryName, CancellationToken cancellationToken)
        {
            return !await _context.CountryMasters
                .AnyAsync(c => EF.Functions.Like(c.CountryName, countryName), cancellationToken);
        }
