    ...
    public class DesignationMasterController : BaseController
    {
        ...
        public async Task<IActionResult> AddDesignationAsync([FromBody] InsertDesignationMasterRequest request)
        {
            ...
            if (!validationResult.IsValid)
            {
                ...
                .FirstOrDefault();
                return BadRequest(errors);// Return a BadRequest response if validation fails
            }
            try
            {
        ...
        [HttpGet("getdesignationidandname")]
        public async Task<IActionResult> GetDesignationIdAndName()
        {
            try
            {
                var result = await designationMaster.GetDesignationIdAndNameFromDB();
                return Ok(result);
            }
            catch (Exception ex)
            {
